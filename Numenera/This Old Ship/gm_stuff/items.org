...
*Effect*: The device scans one object (chosen by the user) that can fit inside a
10-foot (3 m) cube and is within a short distance. The object then dissolves,
its substance drawn into the device. Activating the device again causes it to
create an identical copy of the object using solidified light at an open spot
